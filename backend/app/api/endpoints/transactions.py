...
def find_duplicates(
    ...
    """
    Report-only preview of duplicate_resolution_service's groups (exact
    date+amount+description match, plus a fuzzy cross-source pass ignoring
    description) -- kept for anyone who wants to see what a resolve run
    would touch beforehand. Nothing is deleted here; POST /duplicates/auto-resolve
    (used by the "Solve Duplicities" button) is what actually merges + soft-deletes.
    """
    from app.services.duplicate_resolution_service import find_duplicate_groups

    groups = find_duplicate_groups(db, current_user.id)
    result = [
        {
            "date": g[0].transaction_date.date().isoformat() if g[0].transaction_date else None,
...
def auto_resolve_duplicates(
    ...
    """Automatically merge and soft-delete duplicate transactions for the caller
    (see duplicate_resolution_service.py) -- no per-row review needed up front;
    merged-away rows land in the Recycle Bin, restorable for 30 days, which is
    where a wrong call gets corrected instead of a pre-delete confirmation step.
    Also runs on its own daily via the transactions.auto_resolve_duplicates_all
    Celery Beat task, so this endpoint is for "run it now" rather than the only
    way it happens.
    """
    from app.services.duplicate_resolution_service import resolve_duplicates_for_user

    return resolve_duplicates_for_user(db, current_user.id)
